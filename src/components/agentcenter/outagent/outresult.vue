<template>
  <div class="outresult">
    ...
    <div class="page">
        <el-pagination layout="prev, pager, next, sizes, jumper" :total="this.$store.state.operatorList.length"
...
//    console.log('出口代理商数据', this.$store.state.outListData)
  },
  data () {
    return {
      nowSize: 5,
  ...
  computed: {
    companyList () {
      console.log(this.$store.state.operatorList, 'this.$store.state.outListData')
      if (this.nowPage === 1) {
        return this.$store.state.operatorList.slice(0, this.nowSize)
      } else {
        return this.$store.state.operatorList.slice(((this.nowPage - 1) * this.nowSize), this.nowSize * this.nowPage)
      }
    }
  },
  methods: {
