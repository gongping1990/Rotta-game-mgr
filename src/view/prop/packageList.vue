    ...
    <div class="packageList">
      <el-table stripe :data="getItems">
        ...
        <el-table-column label="操作" align="center" min-width="100">
          <template scope="scope">
            <el-button type="text" class="myBtn" @click="changeStatus(scope.row)">{{scope.row.packageStatus ? '冻结' : '解冻'}}</el-button>
            <el-button type="text" @click="openModal(scope.row)">编辑</el-button>
            <el-button type="text" @click="delPackage(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="page">
...
export default {
  ...
  methods: {
    getPackageList () {
      invoke({
        url: api.packageList.url,
        method: api.packageList.method
      }).then(
        result => {
    ...
    submitProp (id) {
      ...
      this.isSending = true
      invoke({
        url: id ? api.packageUpdate.url : api.addPackage.url,
        method: api.addPackage.method,
    ...
    changeStatus (row) {
      this.$store.commit('startLoading')
      invoke({
        url: api.changePackageStatus.url,
        method: api.changePackageStatus.method,
        data: {
    ...
    delPackage (row) {
      this.$confirm('删除后该礼包无法恢复，你还要继续吗？', `确定删除礼包${row.packageName}吗？`, {
        ...
        invoke({
          url: api.packageDelete.url,
          method: api.packageDelete.method,
          data: {
            packageId: row.packageId,
            packageName: row.packageName
          }
